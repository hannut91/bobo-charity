...
export const signup = async (req: Request, res: Response) => {
    ...
    try {
        const keyPair = await createKeyChain();
        ...
        await faucet(keyPair.address);

        setTimeout(async () => {
            await registerAccount(keyPair.address);
        }, 20000);
    } catch (err) {
        if (newUser) {
            await User.remove({
...
export let updatePassword = async (req: Request, res: Response) => {
    ...
    req.assert('newPassword', 'newPassword must be at least 4 characters long').len({min: 4});
